...
## 01. 프로젝트 주제
아파트 가격 예측을 위한 매매-전세 괴리율 기반 부동산 위험 신호 분석

본 프로젝트는 공지 주제 중 아파트 가격 예측을 기반으로 한다.
다만 단순히 다음 월 가격을 숫자로 맞히는 것에서 끝내지 않고, 가격 예측에 사용되는 시장 신호를 바탕으로 지역별 위험 신호를 해석하는 방향으로 확장하였다.

서울특별시 25개 구의 월별 아파트 매매·전세·월세 데이터를 활용하여, 전세가율, 매매-전세 괴리율, 가격 상승률 차이, 거래량 변화, 월세화 흐름을 가격 예측에 필요한 파생 변수로 생성하였다.

이후 현재 월의 가격·전세·거래량·월세화 지표를 활용해 다음 월 평균 매매가를 예측하고, 예측 가격보다 실제 매매가가 높게 나타나는 지역을 가격을 뒷받침하는 시장 신호에 비해 매매가가 높게 형성된 지역으로 해석하였다.

최종적으로 이 가격 위험 신호를 실거주자·투자자·정체도 관점으로 나누어, 지역별 매수 부담과 고점 매수 위험, 거래 위축 가능성을 분석하였다.
...
flowchart LR 
    ...
    D --> E[가격 예측용 파생 변수 생성] 
    E --> F[다음 월 평균 매매가 예측]
    F --> G[예측 가격 대비 실제 매매가 초과율 확인] 
    G --> H[관점별 위험 신호 해석] 
    H --> I[머신러닝 분류·군집화] 
    I --> J[실거주자·투자자·정체도 관점 결과 해석]
```

본 프로젝트의 흐름은 교수님 공지의 필수 항목인 문제 정의, 데이터 출처, 데이터 전처리, 변수 선택 및 생성 과정, 모델 비교, 결과 해석에 맞추어 구성했다.

특히 본 프로젝트는 가격 예측을 단순한 모델 성능 확인으로 끝내지 않고, 예측 가격과 실제 매매가의 차이를 지역별 가격 위험 신호로 해석하였다.
이후 해당 신호를 실거주자·투자자·정체도 관점으로 나누어 의사결정에 활용할 수 있도록 정리하였다.
---
...
### 05-5. 가격 예측 기반 위험 신호 확인

| 예측 가격 대비 실제 매매가 초과율                                                        | 실제값 vs 예측값                                                                         |
| -------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| <img src="outputs/figures/model/price_support_gap_top10.png" width="100%"> | <img src="outputs/figures/model/price_prediction_actual_vs_pred.png" width="100%"> |

**분석 목적**

현재 월의 가격·전세·거래량·월세화 지표를 활용하여 다음 월 평균 매매가를 예측하였다.
가격 예측의 목적은 단순히 다음 가격을 맞히는 것이 아니라, 현재 매매가가 시장 신호로 설명 가능한 수준인지 확인하는 것이다.
...
따라서 이러한 초과 신호는 실거주자의 매수 부담, 투자자의 고점 매수 위험, 정체 구간의 고평가 가능성을 해석하는 기초 신호로 활용하였다.

**생성 결과 파일**

* `outputs/tables/price_prediction_comparison.csv`
* `outputs/tables/price_prediction_test_predictions.csv`
* `outputs/tables/price_support_gap_summary.csv`
* `outputs/tables/price_support_gap_top10.csv`
* `outputs/figures/model/price_prediction_actual_vs_pred.png`
* `outputs/figures/model/price_support_gap_top10.png`
...
| `05_modeling.ipynb` | 다음 월 평균 매매가 예측, 예측 가격 대비 실제 매매가 초과율 분석, K-Means, 로지스틱 회귀, 랜덤 포레스트 적용 | 가격 예측 성능표, 초과율 그래프, 모델 성능표, 변수 중요도, 군집 요약 생성 |
...
## 12. 가격 예측 및 머신러닝 모델 적용

`05_modeling.ipynb`

본 프로젝트에서는 먼저 현재 월의 가격·전세·거래량·월세화 지표를 활용하여 다음 월 평균 매매가를 예측하였다.
이 가격 예측 결과를 바탕으로 예측 가격보다 실제 매매가가 높게 나타나는 지역을 가격 위험 신호로 해석하였다.

| 모델                      | 사용 목적                       | 해석                                |
| ----------------------- | --------------------------- | --------------------------------- |
| Linear Regression       | 다음 월 평균 매매가 예측을 위한 기본 회귀 모델 | 선형 기준에서 가격 예측 가능성을 확인했다           |
| Random Forest Regressor | 다음 월 평균 매매가 예측 및 비선형 관계 반영  | 변수 간 비선형 관계를 반영하여 가격 예측 성능을 확인했다  |
| K-Means                 | 위험 유형 군집화                   | 지역·월 데이터를 비슷한 위험 특성을 가진 그룹으로 나누었다 |
| Logistic Regression     | 위험 지역 분류                    | 위험 지역과 일반 지역을 구분했다                |
| Random Forest           | 위험 지역 분류 및 변수 중요도 확인        | 분류 성능을 비교하고 위험 판단에 중요한 변수를 확인했다   |

가격 예측은 최종 결과가 아니라, 이후 실거주자·투자자·정체도 관점의 위험 신호 분석으로 확장하기 위한 기준으로 사용했다.
...
2. EDA 결과, 매매가 자체뿐 아니라 전세가·거래량·월세화 흐름이 가격 변동을 설명하는 데 중요한 신호로 판단되었다.
3. 전세가율, 매매-전세 괴리율, 상승률 차이, 거래량 변화율, 월세화 지표를 가격 예측과 위험 해석에 필요한 파생 변수로 생성하였다.
4. 현재 월의 가격·전세·거래량·월세화 지표를 활용하여 다음 월 평균 매매가를 예측하고, 예측 가격 대비 실제 매매가 초과율을 확인하였다.
5. 실제 매매가가 예측 가격보다 높게 나타나는 지역은 가격을 뒷받침하는 시장 신호에 비해 매매가가 높게 형성되었을 가능성이 있다.
6. 이 초과 신호를 바탕으로 실거주자 관점의 매수 부담, 투자자 관점의 고점 매수 위험, 정체도 관점의 고평가 정체 가능성을 해석하였다.
7. K-Means 군집화를 통해 지역·월 데이터를 위험 특성별로 구분할 수 있었고, Random Forest를 통해 위험 판단에 중요한 변수를 확인하였다.
...
본 프로젝트는 매매·전세·월세 실거래 데이터를 기반으로 다음 월 평균 매매가를 예측하고, 예측 가격 대비 실제 매매가의 초과 여부를 활용하여 지역별 가격 위험 신호를 분석했다는 점에서 의미가 있다.

다만 본 분석은 금리, 대출 규제, 개발 호재, 입주 물량, 지역별 소득 수준, 인구 이동 등 외부 요인을 반영하지 못했다.
또한 예측 가격보다 실제 매매가가 높게 나타났다고 해서 해당 지역이 반드시 위험하다고 단정할 수는 없다. 이는 실거주자와 투자자가 추가로 확인해야 할 신호로 해석하는 것이 적절하다.

향후에는 외부 경제 변수와 지역 정보를 추가하여 더 정교한 가격 예측 및 위험 판단 모델로 확장할 수 있다.
